-- Add Staff Tracking and User Performance Tables (Fixed Version)
-- This script adds the necessary tables and columns to track staff assignments and performance
-- It checks for existing columns and only adds missing ones
...
BEGIN
    ...
    -- Check and add staff_id column (only if it doesn't exist)
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'orders' 
        ...
        ALTER TABLE orders ADD COLUMN staff_id UUID REFERENCES users(id) ON DELETE SET NULL;
...
-- Add unique constraints for better data integrity (only if tables exist and columns exist)
DO $$
BEGIN
    -- Check if staff_performance table exists and has the required columns before adding constraint
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'staff_performance' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_performance' AND column_name = 'staff_id' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_performance' AND column_name = 'restaurant_id' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'staff_performance' AND column_name = 'date' AND table_schema = 'public') THEN
        BEGIN
            ALTER TABLE staff_performance ADD CONSTRAINT unique_staff_performance_date UNIQUE (staff_id, restaurant_id, date);
            RAISE NOTICE 'Added unique constraint to staff_performance table';
        EXCEPTION WHEN duplicate_table THEN
            RAISE NOTICE 'Constraint already exists on staff_performance table';
        END;
    END IF;

    -- Check if shift_assignments table exists and has the required columns before adding constraint
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'shift_assignments' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shift_assignments' AND column_name = 'staff_id' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shift_assignments' AND column_name = 'restaurant_id' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shift_assignments' AND column_name = 'shift_date' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'shift_assignments' AND column_name = 'shift_type' AND table_schema = 'public') THEN
        BEGIN
            ALTER TABLE shift_assignments ADD CONSTRAINT unique_shift_assignment UNIQUE (staff_id, restaurant_id, shift_date, shift_type);
            RAISE NOTICE 'Added unique constraint to shift_assignments table';
        EXCEPTION WHEN duplicate_table THEN
            RAISE NOTICE 'Constraint already exists on shift_assignments table';
        END;
    END IF;

    -- Check if financial_tracking table exists and has the required columns before adding constraint
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'financial_tracking' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'financial_tracking' AND column_name = 'restaurant_id' AND table_schema = 'public') AND
       EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'financial_tracking' AND column_name = 'date' AND table_schema = 'public') THEN
        BEGIN
            ALTER TABLE financial_tracking ADD CONSTRAINT unique_financial_tracking_date UNIQUE (restaurant_id, date);
            RAISE NOTICE 'Added unique constraint to financial_tracking table';
        EXCEPTION WHEN duplicate_table THEN
            RAISE NOTICE 'Constraint already exists on financial_tracking table';
        END;
    END IF;
END $$;
