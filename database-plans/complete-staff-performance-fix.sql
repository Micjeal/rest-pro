-- Complete Staff Performance Table Fix
-- This script adds ALL missing columns including timestamps
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added orders_cancelled column';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added revenue_generated column';
    END IF;

    -- Add average_order_value column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'staff_performance' 
    ...
    -- Add total_preparation_time column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'staff_performance' 
        ...
        RAISE NOTICE 'Added total_preparation_time column';
    END IF;

    -- Add average_preparation_time column if it doesn't exist
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'staff_performance' 
        ...
        RAISE NOTICE 'Added average_preparation_time column';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added efficiency_score column';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added shift_type column';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added hours_worked column';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE staff_performance ADD COLUMN created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
    ...
    IF NOT EXISTS (
        ...
        ALTER TABLE staff_performance ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP;
        RAISE NOTICE 'Added updated_at column';
    END IF;
END $$;

-- Verify the table structure after adding all columns
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'staff_performance' 
AND table_schema = 'public'
...
-- Now populate with sample data (only if table is empty)
...
AND NOT EXISTS (
  SELECT 1 FROM staff_performance sp 
